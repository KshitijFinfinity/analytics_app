...
export default function FunnelsPage() {
  ...
  function addFunnelStep(eventName) {
    if (!eventName) return;
    setFunnelBuilder((prev) => ({ ...prev, steps: [...prev.steps, eventName] }));
  }
  ...
  function removeFunnelStep(index) {
    setFunnelBuilder((prev) => {
      ...
      return { ...prev, steps: next.length > 0 ? next : ["page_view"] };
    });
  }
      ...
      {funnelMessage ? <p className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-700">{funnelMessage}</p> : null}
            ...
            <div className="pt-2">
              ...
              <div className="space-y-2 relative">
                ...
                <div className="relative z-10 flex items-center gap-2 mt-4 ml-8">
                   <select value="" onChange={(e) => { addFunnelStep(e.target.value); e.target.value = ""; }} className="flex-1 rounded-lg border border-dashed border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-500 outline-none hover:border-slate-400 hover:bg-white transition-colors cursor-pointer">
        ...
        <div className="flex flex-col gap-6">
          <Card className="p-0 overflow-hidden flex flex-col h-[400px]">
            ...
            <div className="flex-1 p-5 bg-slate-50/50 flex flex-col relative overflow-hidden">
              {funnelLoading && (
